export default function aria ($elem = document.body) {
	return {
		get (name) {
		...
		find (name, value?) {
			return value
				? $elem.querySelector(`[aria-${name}=${value}]`)
				: $elem.querySelector(`[aria-${name}]`)
		},
		findAll (name, value?) {
			return value
				? $elem.querySelectorAll(`[aria-${name}=${value}]`)
				: $elem.querySelectorAll(`[aria-${name}]`)
		},
		controlTarget () {
